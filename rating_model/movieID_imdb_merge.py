...
df_mt['releaseYear'] = df_mt['releaseYear'].apply(lambda x: -999
                                                  if x == 'NULL' else int(x))
df_mt["title"] = df_mt['movieID'].apply(lambda x: x[2])
df_mt["movieID"] = df_mt['movieID'].apply(lambda x: x[0])

df_mt['type'] = df_mt["title"].apply(
    lambda x: "tv show"
    if ": series " in x.lower() or ": season " in x.lower() else "movie")
df_mt['title'] = df_mt['title'].apply(lambda x: x[:x.lower().find(
    ": s")] if ": series " in x.lower() or ": season " in x.lower() else x)

#Add genres to the dataset based on IMDB data

mt_genres['startYear'] = mt_genres['startYear'].apply(
    lambda x: -999 if x == '\\N' else int(x))
mt_genres['to_drop'] = mt_genres['startYear'].apply(lambda x: 1
                                                    if int(x) > 2005 else 0)
mt_genres = mt_genres[mt_genres['to_drop'] != 1]
mt_genres.drop('to_drop', axis=1, inplace=True)

df_mt_genres = pd.merge(df_mt,
                        mt_genres,
                        how='left',
                        left_on=["title", 'type'],
...
df_mt_genres['no_cat'] = df_mt_genres['genres'].apply(
    lambda x: len(str(x).split(",")))

df_mt_genres.sort_values(['movieID', 'startYear', 'numVotes', 'no_cat'],
                         ignore_index=True,
                         inplace=True,
                         ascending=True)
...
for i in range(len(indices)):
    c_movies = len(df_mt_genres[
        (df_mt_genres['movieID'] == df_mt_genres['movieID'][indices[i]])
        & (df_mt_genres['to_drop'] == 0)])
    count = c_movies
    if df_mt_genres['releaseYear'][indices[i]] == df_mt_genres['startYear'][
            indices[i]]:
        df_mt_genres['to_keep'][indices[i]] = 1
        continue
    else:
        if (count > 1) and (df_mt_genres['releaseYear'][indices[i]] != 0):
            if df_mt_genres['startYear'][
                    indices[i]] > df_mt_genres['releaseYear'][indices[i]]:
                df_mt_genres['to_drop'][indices[i]] = 1
                count -= 1
        if (count > 1) and len(df_mt_genres[['movieID', 'to_drop']][
            (df_mt_genres['movieID'] == df_mt_genres['movieID'][indices[i]])
                & (df_mt_genres['to_drop'] == 0)]) > 1:
            if not df_mt_genres[[
                    'movieID', 'numVotes', 'to_drop'
            ]][(df_mt_genres['movieID'] == df_mt_genres['movieID'][indices[i]])
               & (df_mt_genres['to_drop'] == 0)
               & (df_mt_genres['numVotes'] >
                  df_mt_genres['numVotes'][indices[i]])].empty:
                df_mt_genres['to_drop'][indices[i]] = 1
...
for i in range(len(indices_duplicates)):
    ...
    if df_mt_genres['to_drop'][ix] == 0 and df_mt_genres['to_keep'][ix] == 0:
        if len(df_mt_genres[
            (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                & (df_mt_genres['to_drop'] == 0) &
            (df_mt_genres['to_keep'] == 1)]) >= 1:
            df_mt_genres['to_drop'][ix] = 1
    if df_mt_genres['to_drop'][ix] == 0 and len(df_mt_genres[
        (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
            & (df_mt_genres['to_drop'] == 0)]) > 1:
        if len(df_mt_genres[
            (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
            ...
            if df_mt_genres['to_keep'][ix] == 0:
                df_mt_genres['to_drop'][ix] = 1
        elif len(df_mt_genres[
            (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                & (df_mt_genres['to_keep'] == 1)]
                 ) > 1 and df_mt_genres['to_keep'][ix] == 1:
            if not df_mt_genres[
                (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                    & (df_mt_genres['to_drop'] == 0)
                    & (df_mt_genres['to_keep'] == 1) &
                (df_mt_genres['numVotes'] >
                 df_mt_genres['numVotes'][ix])].empty:
                df_mt_genres['to_drop'][ix] = 1
        elif len(df_mt_genres[
            (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                & (df_mt_genres['to_keep'] == 1)]) == 0:
            if not df_mt_genres[
                (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                    & (df_mt_genres['to_drop'] == 0)
                    & (df_mt_genres['numVotes'] >
                       df_mt_genres['numVotes'][ix])].empty:
                df_mt_genres['to_drop'][ix] = 1
            elif not df_mt_genres[
                (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                    & (df_mt_genres['to_drop'] == 0)
                    &
                ((df_mt_genres['releaseYear'] - df_mt_genres['startYear']) <
                 (df_mt_genres['releaseYear'][ix] -
                  df_mt_genres['startYear'][ix]))].empty:
                df_mt_genres['to_drop'][ix] = 1
        if df_mt_genres['to_keep'][ix] == 0 and df_mt_genres['to_drop'][
                ix] == 0:
            if len(df_mt_genres[
                (df_mt_genres['movieID'] == df_mt_genres['movieID'][ix])
                    & (df_mt_genres['to_drop'] == 0) &
                (df_mt_genres['to_keep'] == 0)]) > 1:
                df_mt_genres['to_drop'][ix] = 1
