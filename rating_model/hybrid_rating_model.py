...
df['movie'] = df.type.apply(lambda x: 1
                            if 'movie' in x else 0)  # content type encoding
...
for genre in genres:
    df[genre] = df['genres'].apply(
        lambda x: 1 if genre in x else 0)  # encoding of "genres"
...
X_train, X_test, y_train, y_test = train_test_split(df.drop(["rating"],
                                                            axis=1),
                                                    df['rating'],
                                                    test_size=0.33,
                                                    random_state=1)
...
user_movie_df = train.pivot(index='userID', columns='movieID',
                            values='rating').fillna(0)  # user-item matrix
...
def find_neighbors(userID, movieID, n_neighbors):
    ...
    if movieID in train_movieIDs:
        ...
        users_ix.append(
            userID
        )  # gives indices of users who rated movie and user from test set
        if userID in userIDs:
            ...
            user_sim = out[0][0].tolist(
            )  # cosine distance of the neighbors to userID
            sim_users = []
            data['userID'] = data.index
            for _ in range(1,
                           len(user_ix)):  # to exclude userID from the output
...
def calc_rating(sim_users, movieID):
    ...
    if len(sim_users) > 1:
        ...
        for i in sim_users:
            user_id = i[0]
            cos_sim_user = 1 - i[1]
            movie_rating = train.loc[(train.movieID == movieID) &
                                     (train.userID == user_id),
                                     'rating'].values
            if len(movie_rating) > 0:
                movie_rating = movie_rating[
                    0]  # rating of movieID by a similar user
            weighted_rating_sum += (
                movie_rating * (cos_sim_user)
            )  # gives sum of movieID rating multiplied by cosine similarity of particular user
...
X_test['similar_users'] = X_test.apply(
    lambda x: find_neighbors(x['userID'], x['movieID'], 10),
    axis=1)  # top 10 similar users with corresponding cosine distance
X_test['user_based_rating'] = X_test.apply(
    lambda x: calc_rating(x['similar_users'], x['movieID']),
    axis=1)  # returns weighted average rating of movieID
...
def rate_movies(userID, test_row):
    ...
    userIDs = movie_df.userID.unique(
    )  # which userIDs are in the training set?
...
X_test['taste_based'] = X_test.apply(
    lambda row: rate_movies(row['userID'], row), axis=1
)  # returns predicted rating value of movie based on user preferences
X_test['taste_based'] = X_test['taste_based'].fillna(0).apply(
...
X_test['avr_movie_rating'] = X_test.movieID.apply(
    lambda x: avr_movie_rat.loc[x, 'rating']
    if x in avr_movie_rat.index else "NA")  # returns average rating of movie
...
y_pred = X_test[[
    'user_based_rating', 'taste_based', 'avr_movie_rating', 'lin_reg'
]]
...
def final_rating(row):
    ...
    for _ in range(len(row)):
        if type(row[_]
                ) == float and row[_] > 0 and row[_] <= 5 and row[_] != "NA":
            sum_rates += row[_]
            n_rates += 1
